...
def generate_class_source(d):
    ...
    assignment_exprs = '\n'.join([constructor_assignment_exprs_template.format(attribute=p.name) for p in d.properties])
    ...
    # Write methods
    pack_args = '\n'.join([pack_args_template.format(name=snake_case(d.name), attribute=p.name) for p in d.properties])
